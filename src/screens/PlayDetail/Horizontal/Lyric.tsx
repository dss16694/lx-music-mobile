...
import { type Line, useLrcPlay, useLrcSet, useWordLrcPlay, useWordLrcSet, type WordLine, type WordPlayInfo } from '@/plugins/lyric'
...
export default () => {
  ...
  // ── 逐字模式 ──────────────────────────────────────────────────
  if (useWordMode) {
    const renderWordItem: WordFlatListType['renderItem'] = ({ item, index }) => {
      const extendedLyrics = lyricLines[index]?.extendedLyrics ?? []
      return (
        <WordLrcLine
          line={item}
          lineNum={index}
    ...
    const getWordKey: WordFlatListType['keyExtractor'] = (item, index) => `w${index}${item.text}`
    ...
    return (
      <>
        <FlatList
          ...
          initialNumToRender={Math.max(wordPlayInfo.line + 10, 10)}
